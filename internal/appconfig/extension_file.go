...
type ExtensionField struct {
	Name      string            `yaml:"name"`     // field name as generated by default: "AssetsFs"
	Overwrite string            `yaml:"override"` // field name to use in the final struct: "AssetsFS"
	Type      string            `yaml:"type"`
	Setter    bool              `yaml:"setter"`
	Tags      map[string]string `yaml:"tags"`
}

// ExtensionConfig maps a normalized key to its list of extension fields
...
func processRawConfig(prefix string, raw map[string]any, config ExtensionConfig) {
	for k, v := range raw {
		...
		// support arrays by adding an array identifier
		isArrayPath := strings.HasSuffix(k, "[*]")
		...
		case []any:
			...
			if isArrayPath {
				structName := extractStructNameFromPath(path)
				config[path] = fields
				config[normalizeKey(structName)] = fields
			} else {
				config[path] = fields
			}
